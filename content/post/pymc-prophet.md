...
$$ \vec{g} = (k + A \delta) \odot t + (m + A \gamma) \tag{6}$$ 
...
Above we see the result of the piecewise linear trend with changepoints. The changepoints are shown by vertically dashed lines. The first plot shows the result of the whole trend function as a reasonable continuous trend. The middle plot show the growth rate $ k + \sum\_{j: t > s\_j} \delta\_j $ at time $t$. The last plot shows the offset at time $t$. These are just horizontal plateaus that adjust the growth rate's height in order to generate a continuous plot.
...
Let's also load and prepare some data. We are going to use the same dataset as Facebook uses in their [Quick Start](https://facebook.github.io/prophet/docs/quick_start.html) tutorial of Prophet. The _csv_ file can be [downloaded here](https://github.com/facebook/prophet/blob/master/examples/example_wp_log_peyton_manning.csv).
...
df.plot(x='ds', y='y', figsize=(16, 6), title="Wikipedia page views for 'Peyton Manning'")
```

{{< figure src="/img/post-19-prophet/manning-ts.png" title="Daily page views of Peyton Manning's Wikipedia page." >}}
...
def trend_model(m, t, n_changepoints=25, changepoints_prior_scale=0.05, 
    ...
    with m:
        ...
        if changepoints_prior_scale is None:
            changepoints_prior_scale = pm.Exponential('tau', 0.5)
...
{{< figure src="/img/post-19-prophet/bimodal.png" title="Bimodal distribution, where the mode is in a different region than the mean." >}}

Nonetheless, we are going to estimate the parameters of our model with MAP. Later in this post we will sample the posterior distribution and look at the benefits this yields.
...
Above we see the result of our parameter estimation, which looks pretty reasonable. We see that the model has remained a quite linear trend. This is due to the prior we have set on the changepoint\_prior, $\tau = 0.05$. If we would increase this value, the model would have more flexibility and be able to follow the seasonality better, but as a result, have a higher probability of overfitting to the data.

## Seasonality
Of course, we cannot forecast business time series, without modelling seasonalities. Prophet models seasonalities for daily, weekly, monthly and yearly patters, all based on Fourier series. In this post we will only focus on weekly and yearly seasonality, but the adding more will be easy!
...
## Complete model
Now we've set up all we need for our complete model. We mentioned earlier that Prophet, estimateds the a maximum point estimate of the posterior distribution. We are going to sample from the posterior. By sampling, we are will not find values for our parameters, but find whole distributions! This is great! We will have a lot of information about the uncertainty of our predictions by analyzing the posterior distributions, or by just sampling the posterior! I think this is
very cool, and see this as one of the best properties of Bayesian modelling.

Because we will be sampling, we can set a hierarchical prior on the changepoint\_prior\_scale $\tau$. In the base model, this was set to $\tau = 0.05$.
